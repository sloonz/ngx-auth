...
export const loginPage = (providers: Record<string, { url: string; desc: string; }>) => <html>
	<head>
		<title>Login</title>
		<style>{`
			a {
	...
	<body>
		{Object.entries(providers).map(([provider, { url, desc }]) => <a key={provider} href={url}><span><img src={icons[provider]} height="20" /></span> Login with {desc}</a>)}
	</body>
</html>;
